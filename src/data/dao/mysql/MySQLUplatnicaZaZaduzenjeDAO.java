 ...
 * @author DulleX
 */
public class MySQLUplatnicaZaZaduzenjeDAO implements UplatnicaZaZaduzenjeDAO {

    @Override
    public List<UplatnicaZaZaduzenjeDTO> zaduzenjaStudenta(String JMB) {
        List<UplatnicaZaZaduzenjeDTO> retVal = new ArrayList<>();
        ...
        ResultSet rs = null;
         ...
         String query = "SELECT u.IdUplatnice, DatumUplate, IznosUplate, Username, BrojSobe, BrojPaviljona, JMB, SkolskaGodina  "
                + "FROM uplatnica u"
                + "INNER JOIN uplatnica_za_zaduzenje k ON u.IdUplatnice = k.IdUplatnice "
                + "WHERE k.JMB = ? ";
        ...
        try {
            ...
            while(rs.next())
            {
                 retVal.add(new UplatnicaZaZaduzenjeDTO(rs.getInt("IdUplatnice"), rs.getDate("DatumUplate"),
    ...
    public List<UplatnicaZaZaduzenjeDTO> svaZaduzenja() {
        ...
        String query = "SELECT IdUplatnice, DatumUplate, IznosUplate, Username, JMB, BrojSobe, BrojPaviljona, SkolskaGodina  "
                + "FROM uplatnica u"
                + "INNER JOIN uplatnica_za_zaduzenje USING(IdUplatnice) "
    ...
    public boolean dodaj(UplatnicaZaZaduzenjeDTO uplatnica) {
        ...
        String query = "INSERT INTO uplatnica_za_zaduzenje(IdUplatnice, JMB, BrojSobe, BrojPaviljona, SkolskaGodina) VALUES "
                + "(?, ?, ?, ?, ?) ";
        try {
            conn = ConnectionPool.getInstance().checkOut();
            ps = conn.prepareStatement(query);
    ...
    public boolean azuriraj(UplatnicaZaZaduzenjeDTO uplatnica) {
        ...
        String query2 = "UPDATE uplatnica_za_zaduzenje SET "
                + "JMB=?, "
                + "BrojSobe=?, "
                + "BrojPaviljona=? "
                + "SkolskaGodina=? "
    ...
    public boolean obrisi(UplatnicaZaZaduzenjeDTO uplatnica) {
        ...
        String query = "DELETE FROM uplatnica_za_zaduzenje "
                + "WHERE IdUplatnice=? ";
        try {
            conn = ConnectionPool.getInstance().checkOut();
            ps = conn.prepareStatement(query);
