 ...
 * @author DulleX
 */
public class MySQLUplatnicaZaStanarinuDAO implements UplatnicaZaStanarinuDAO{

    @Override
    public List<UplatnicaZaStanarinuDTO> stanarineStudente(String JMB) {
        List<UplatnicaZaStanarinuDTO> retVal = new ArrayList<UplatnicaZaStanarinuDTO>();
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;


         String query = "SELECT IdUplatnice, DatumUplate, IznosUplate, Username, JMB, BrojSobe, Mjesec, BrojPaviljona, SkolskaGodina  "
                + "FROM uplatnica u"
                + "INNER JOIN uplatnica_za_stanarinu k ON u.IdUplatnice = k.IdUplatnice "
                + "WHERE IdUplatnice = ? ";
        ...
        try {
            ...
            while(rs.next())
            {
                 retVal.add(new UplatnicaZaStanarinuDTO(rs.getInt("IdUplatnice"), rs.getDate("DatumUplate"),
                        rs.getFloat("IznosUplate"), rs.getString("Username"), rs.getString("JMB"), rs.getInt("BrojSobe"), rs.getInt("Mjesec"), rs.getShort("BrojPaviljona"), rs.getString("SkolskaGodina")));
            }
    ...
    public List<UplatnicaZaStanarinuDTO> sveStanarine() {
        ...
        String query = "SELECT IdUplatnice, DatumUplate, IznosUplate, Username, JMB, BrojSobe, Mjesec, BrojPaviljona, SkolskaGodina  "
                + "FROM uplatnica u"
                + "INNER JOIN uplatnica_za_stanarinu USING(IdUplatnice) "
    ...
    public boolean dodaj(UplatnicaZaStanarinuDTO uplatnica) {
        ...
        String query = "INSERT INTO uplatnica_za_stanarinu(IdUplatnice, JMB, BrojSobe, Mjesec, BrojPaviljona, SkolskaGodina) VALUES "
                + "(?, ?, ?, ?, ?, ?) ";
        try {
            conn = ConnectionPool.getInstance().checkOut();
            ps = conn.prepareStatement(query);
            ...
            ps.setInt(4,uplatnica.getMjesec());
            ps.setShort(5, uplatnica.getBrojPaviljona());
            ps.setString(6, uplatnica.getSkolskaGodina());
    ...
    public boolean azuriraj(UplatnicaZaStanarinuDTO uplatnica) {
        ...
        String query2 = "UPDATE uplatnica_za_stanarinu SET "
                + "JMB=?, "
                + "BrojSobe=?, "
                + "BrojPaviljona=? "
                + "Mjesec=? "
        ...
        try {
            ...
            ps2.setInt(4, uplatnica.getMjesec());
            ps2.setString(5, uplatnica.getSkolskaGodina());
            ps2.setInt(6, uplatnica.getIdUplatnice());
    ...
    public boolean obrisi(UplatnicaZaStanarinuDTO uplatnica) {
        ...
        String query = "DELETE FROM uplatnica_za_stanarinu "
                + "WHERE IdUplatnice=? ";
        try {
            conn = ConnectionPool.getInstance().checkOut();
            ps = conn.prepareStatement(query);
